...
const fetchData = async () =>{
    const rspns = await fetch('/friends.json')
    return rspns.json();
}
    ...
    return (
        <div className='bg-gray-50'>
            <div className='max-w-[80%] mx-auto pt-10 pb-13 sm:pt-16'>
                ...
                <div className='py-5 border-b border-gray-200 mb-5 grid min-[380px]:grid-cols-2 md:grid-cols-4 gap-6'>
                    <div className='text-center py-6 rounded-xl bg-white shadow space-y-1'>
                        <h1 className='text-[#244D3F] text-2xl font-bold'>{friends.length}</h1>
                        <p className='text-gray-400 text-sm'>Total Friends</p>
                    </div>
                    <div className='text-center py-6 rounded-xl bg-white shadow space-y-1'>
                        <h1 className='text-[#244D3F] text-2xl font-bold'>{friends.filter(f=>f.status == "on-track").length}</h1>
                        <p className='text-gray-400 text-sm'>On Track</p>
                    </div>
                    <div className='text-center py-6 rounded-xl bg-white shadow space-y-1'>
                        <h1 className='text-[#244D3F] text-2xl font-bold'>{friends.filter(f=>f.status == "overdue").length}</h1>
                        <p className='text-gray-500 text-sm'>Need Attention</p>
                    </div>
                    <div className='text-center py-6 rounded-xl bg-white shadow space-y-1'>
                        <h1 className='text-[#244D3F] text-2xl font-bold'>{friends.filter(f=>f.status == "almost due").length+friends.filter(f=>f.status == "overdue").length}</h1>
                        <p className='text-gray-500 text-sm'>Interactions This Month</p>
                    </div>
                </div>
                <h3 className='font-semibold text-2xl text-gray-700 mb-2'>Your Friends</h3>
