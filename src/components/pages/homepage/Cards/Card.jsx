...
const Card = ({friends}) => {
    return (
        <div className='grid min-[557px]:grid-cols-2 min-[939px]:grid-cols-3 min-[1261px]:grid-cols-4 gap-6'>
            {
                friends.map(friend=>(
                    <Link to={`/details/${friend.id}`} key={friend.id} className='border border-gray-50 bg-white shadow flex flex-col items-center justify-center px-3 py-6 rounded-xl'>
